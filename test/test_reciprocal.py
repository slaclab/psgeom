...
class TestGeometry(object):
    ...
    def test_recpolar_n_reciprocal(self):
        q1 = np.linalg.norm(self.d.reciprocal, axis=-1)
    ...
    def test_polar_space(self):
        ...
        np.testing.assert_array_almost_equal(yy, np.squeeze(x))
        np.testing.assert_array_almost_equal(xx, np.squeeze(y))
    ...
    def test_geom_compat(self):
        geom = camera.CompoundAreaCamera.from_crystfel_file('ref_files/jungfrau/jungfrau-1M-pan.geom', 
                                                            element_type=sensors.JungfrauSegment)
        eV = 9500.0
        d = reciprocal.Geometry(geom, eV)
        assert d.xyz.shape == (2, 512, 1024, 3)
